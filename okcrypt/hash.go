...
func GetFileSha256Hash(filePath string) (string, error) {
	...
	if err != nil {
		oklog.Errorf("error on opening file %v\n%v", filePath, err)
		return "", err
	}
	defer f.Close()
	...
	if _, err := io.Copy(h256, f); err != nil {
		oklog.Errorf("error on calculating file %v hash\n%v", filePath, err)
		return "", err
	}

	hash := hex.EncodeToString(h256.Sum(nil))
	oklog.Debugf("file %v SHA256 hash %v", filePath, hash)
...
func GetFileMd5Hash(filePath string) (string, error) {
	...
	oklog.Debugf("file %v MD5 hash %v", filePath, hash)
...
func SignWithSecretKey(key string, inputs ...string) (string, error) {
	...
	if err != nil {
		oklog.Errorf("error on hmac inputs %v\n%v", message, err)
		return "", err
	}
